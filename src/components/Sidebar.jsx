...
import { TbLayoutSidebarLeftCollapseFilled, TbSquareRounded } from "react-icons/tb";
...
const Sidebar = memo(({sidebar, closeSidebar}) => {
    ...
    useEffect(() => {
        ...
        setBaseLink(pathParts[0] || "");
        setSubLink(pathParts[1] || "");
        setLastLink(pathParts[2] || "");
    }, [loc.pathname]);
    ...
    return (
        <div className={`${sidebar ? "z-20 w-screen max-sm:fixed top-0 left-0 backdrop-blur-lg sm:w-[30%] lg:w-1/4 xl:w-1/6 sm:shadow-[4px_0px_10px_0px_#B3B3B366] " : " shadow-none w-0"} py-5  min-h-screen overflow-x-hidden trans flex flex-col gap-5 `}>
            {<div className={`w-full ${sidebar ? "px-5" : "px-0"} trans border-b-2`}>
                <button onClick={closeSidebar} className=" h-10 w-10 mb-5 rounded-md trans hover:bg-gray-200 flex items-center justify-center">
                            ...
                            links.map((link, index) => (
                                <div key={link.id} className='relative w-full flex flex-col gap-1'>
                                    <button to={link.link} onClick={() => handleOpenList(index)}  className={`outline-0 hover:pl-2  ${baseLink == link.link ? "pl-2 beforeElement bg-main-color text-white" : openList[index] ? "bg-gray-200 pl-2" : " hover:bg-gray-200"} flex w-full justify-between items-center    py-2 trans rounded-md font-semibold `}>
                                        ...
                                        <FaAngleDown  className={`mt-1 mr-1 ${openList[index]  ? "rotate-180" : ""} trans`}/>
                                    </button>
                                    ...
                                    className={`flex flex-col gap-1 pl-2 text-sm trans overflow-y-hidden`}>
                                        {
                                            link.children.map((child, index) => (
                                                <li key={child.id} className='flex-col cursor-pointer trans flex items-center justify-between'>
                                                    <Link to={child.children && child.children.length > 0 ? loc.pathname : child.link} onClick={() => {child.children ? handleCategoryOpenList(index) :  (window.innerWidth < 768) &&  closeSidebar() }} className={`w-full  trans py-1 ${ subLink == child.linkCompare ? "text-main-color pl-1" : " "} hover:bg-gray-200 hover:pl-1 rounded-sm outline-0 flex items-center justify-between`}>
                                                        <p className='flex gap-1 items-center   '>
                                                            <RxDot className={`text-xs  mt-0.5 `} />
                                                            {child.text}
                                                        </p>
                                                        {child.children && <FaAngleDown  className={`mt-1 mr-1 ${ categoryOpenList[index] ? "rotate-180" : "rotate-0 "} trans `}/>}
                                                    </Link>
